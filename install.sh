...
if [ 0 == $UID ]; then
    ...
    echo -e "The current user is not the root user. Please use the sudo su command to switch to the root user and re-execute the script."
    exit 1
fi
...
if [[ -f /usr/bin/vlmcsd ]]; then
    ...
    echo -e "vlmcsd updated successfully"
    exit 0
fi
...
echo -e "vlmcsd installed successfully"
